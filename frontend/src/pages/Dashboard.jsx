...
import { Users, TrendingUp, Calendar, MessageSquare, ArrowUpRight, RefreshCw, AlertCircle } from 'lucide-react';
...
const StatSkeleton = () => (
  <div className="glass-panel p-6 rounded-2xl animate-pulse">
    <div className="flex justify-between items-start mb-4">
      <div className="w-12 h-12 rounded-xl bg-dark-200 dark:bg-dark-700" />
      <div className="w-16 h-6 rounded-full bg-dark-200 dark:bg-dark-700" />
    </div>
    <div className="h-4 w-24 rounded bg-dark-200 dark:bg-dark-700 mb-2" />
    <div className="h-8 w-16 rounded bg-dark-200 dark:bg-dark-700" />
  </div>
);
...
// Dados fixos do gráfico (placeholder visual — receita real requer tabela de faturamento)
const chartData = [
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      {error && (
        <div className="flex items-center gap-3 p-4 rounded-xl bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400">
          ...
          <span className="text-sm">Erro ao carregar métricas: {error}</span>
        </div>
      )}
      ...
      <div className="glass-panel p-6 rounded-2xl">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-bold text-dark-900 dark:text-white">Atendimentos da Semana</h3>
          <span className="text-xs text-dark-400 dark:text-dark-500 bg-dark-100 dark:bg-dark-800 px-3 py-1 rounded-full">
            Estimativa visual
          </span>
        </div>
        <div className="h-[280px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              />
              <Area type="monotone" dataKey="atendimentos" stroke="#22c55e" strokeWidth={3} fillOpacity={1} fill="url(#colorApt)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
